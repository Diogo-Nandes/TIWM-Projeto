...
export default function PerfilScreen() {
  ...
  const [nomeAssociado, setNomeAssociado] = useState<string | null>(null);
    ...
    const subscriber = firestore()
      ...
      .onSnapshot(async querySnapshot => {
        if (!querySnapshot.empty) {
          ...
          // Se existir um utilizador associado, vai buscar o nome
          if (data.uidAssociado) {
            try {
              const associadoSnap = await firestore()
                .collection('Utilizadores')
                .where('uid', '==', data.uidAssociado)
                .limit(1)
                .get();
              if (!associadoSnap.empty) {
                setNomeAssociado(associadoSnap.docs[0].data().Username || null);
              } else {
                setNomeAssociado(null);
              }
            } catch (err) {
              setNomeAssociado(null);
            }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {/* BOTÃO DE ASSOCIAR UTILIZADOR */}
      <TouchableOpacity
        style={styles.buttonAssociate}
        onPress={() => router.push('/(cuidador)/pesquisa-user')}
        accessibilityRole="button"
      >
        <Ionicons name="person-add" size={24} color="#fff" style={{ marginRight: 8 }} />
        <Text style={styles.buttonTextAssociate}>
          {nomeAssociado ? `${nomeAssociado}` : 'Associar Utilizador'}
        </Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  placeholderContent: { flex: 1, width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center' },
  placeholderText: { color: '#888', fontSize: 18, marginTop: 8, fontWeight: 'bold', textAlign: 'center' },
